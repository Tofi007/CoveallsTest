...
public class WordCountPerformanceMutableForward {
		...
		if (args != null && args.length == 9) {
			try {
				...
				DataStream<Tuple2<String, Integer>> dataStream = env
						...
						.flatMap(new WordCountPerformanceSplitter()).setParallelism(splitterSize).setMutability(true)
							.partitionBy(0)
						.map(new WordCountPerformanceCounter()).setParallelism(counterSize).forward().setMutability(true)
						.addSink(new WordCountPerformanceSink(args, csvPath)).setMutability(true)
